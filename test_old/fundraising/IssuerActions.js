...
contract('Issuer actions', async function ([
  _,
  whitelistManager,
  owner,
  issuer,
  ...
  beforeEach(async function () {
    ...
    await this.swarmPoweredFundraiseMock.setContributorRestrictions(
    ...
    this.contributorRestrictionsFinished = await ContributorRestrictions.new(
      this.swarmPoweredFundraiseFinished.address,
      {from: issuer}
    );
    await this.swarmPoweredFundraiseFinished.setContributorRestrictions(
      this.contributorRestrictionsFinished.address
    );
    ...
    this.contributorRestrictionsCanceled = await ContributorRestrictions.new(
      this.swarmPoweredFundraiseCanceled.address,
      {from: issuer}
    );
    await this.swarmPoweredFundraiseCanceled.setContributorRestrictions(
      this.contributorRestrictionsCanceled.address
    );
    ...
    this.contributorRestrictionsExpired = await ContributorRestrictions.new(
      this.swarmPoweredFundraiseExpired.address,
      {from: issuer}
    );
    await this.swarmPoweredFundraiseExpired.setContributorRestrictions(
      this.contributorRestrictionsExpired.address
    );
  });
  ...
  describe('Fundraising whitelist functionality', () => {
    ...
    it('should not be able to whitelist as not-authorized account', async function () {
      await shouldFail.reverting.withMessage(
        this.contributorRestrictions.whitelistAccount(contributor, {from: contributor}),
    ...
    it('should be able for contribution to automatically be moved to accepted contributions if contributor has been whitelist', async function () {
      const beforeBalanceETH = await this.swarmPoweredFundraiseMock.getBalanceETH(contributor, {
        from: issuer,
      });
      // contribute
      ...
      const afterBalanceETH = await this.swarmPoweredFundraiseMock.getBalanceETH(contributor, {
        from: issuer,
      });
      ...
      const beforeHistoricalBalances = await this.swarmPoweredFundraiseMock.getHistoricalBalanceETH(
        sequence - 1,
        {from: issuer}
      );
      const afterHistoricalBalances = await this.swarmPoweredFundraiseMock.getHistoricalBalanceETH(
        sequence,
        {from: issuer}
      );
      ...
      const accContribution = await this.swarmPoweredFundraiseMock.isContributionAccepted(
        sequence,
        {from: issuer}
      );
    ...
    it('should not be able to de-whitelist as non-authorized account', async function () {
      await this.contributorRestrictions.whitelistAccount(contributor, {from: issuer});
      await shouldFail.reverting.withMessage(
        this.contributorRestrictions.unWhitelistAccount(contributor, {from: contributor}),
        'Ownable: caller is not the issuer or manager'
      );
    });
    ...
    it('should not be able to whitelist if fundraising is finished', async function () {
      await shouldFail.reverting.withMessage(
        this.contributorRestrictionsFinished.whitelistAccount(contributor, {from: issuer}),
    ...
    it('should not be able to whitelist if fundraising is expired', async function () {
      await shouldFail.reverting.withMessage(
        this.contributorRestrictionsExpired.whitelistAccount(contributor, {from: issuer}),
    ...
    it('should not be able to whitelist if fundraising is canceled', async function () {
      await shouldFail.reverting.withMessage(
        this.contributorRestrictionsCanceled.whitelistAccount(contributor, {from: issuer}),
    ...
    it('should not be able to de-whitelist if fundraising is finished', async function () {
      await shouldFail.reverting.withMessage(
        this.contributorRestrictionsFinished.unWhitelistAccount(contributor, {from: issuer}),
    ...
    it('should not be able to de-whitelist if fundraising is expired', async function () {
      await shouldFail.reverting.withMessage(
        this.contributorRestrictionsExpired.unWhitelistAccount(contributor, {from: issuer}),
    ...
    it('should not be able to de-whitelist if fundraising is canceled', async function () {
      await shouldFail.reverting.withMessage(
        this.contributorRestrictionsCanceled.unWhitelistAccount(contributor, {from: issuer}),
