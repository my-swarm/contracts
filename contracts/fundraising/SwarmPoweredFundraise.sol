...
contract SwarmPoweredFundraise {
    ...
    event ContributionReceived(address indexed account, uint amount);
    ...
    uint public endDate;
    uint public expirationTime = 2592000; // default: 60 * 60 * 24 * 30 = ~1month
    uint public fee = 2000e6; // USDC has 6 decimal places
    ...
    uint public fundraiseAmount;

    // variables that keep track of state
    uint public numberOfContributors;
    uint public totalFundsWithdrawal;
    uint public qualifiedSums;
    uint public bufferedSums;
    ...
    mapping(address => uint) public totalAffiliateClaim;

    // funds directed to the affiliate from referrals per contributor
    mapping(address => uint) public affiliateClaim;
    ...
    */
    function acceptContributor(
      ...
      returns (bool)
    {
      uint amount = bufferedContributions[contributor];
      // process contribution
      if(amount > 0) {
        bufferedContributions[contributor] = 0;
        bufferedSums = bufferedSums.sub(amount);
        string memory link = IAffiliateManager(affiliateManager).getAffiliateLink(referral[contributor]);
    ...
    */
      ...
      returns (bool)
    {
      ...
      uint amountToMint = _finishFundraise();
    ...
    */
    function claimTokens() external returns (bool) {
      ...
      require(
        IERC20(token).transfer(msg.sender, tokenShares),
    ...
    function claimReferrals() external returns (bool) {
      ...
      uint amount = totalAffiliateClaim[msg.sender];
    ...
    */
      ...
      returns (bool)
    {
      ...
      if (IContributorRestrictions(contributorRestrictions).isWhitelisted(contributor)) {
        ...
        uint refund  = _updateContributions(contributor, amount, qualifiedContributions, qualifiedSums);
        (bool pass, uint overAmount) = _checkHardCap(qualifiedContributions[contributor]);
        if(overAmount > 0 || refund > 0) {
          uint totalRefund = overAmount.add(refund);
        ...
        _updateContributions(contributor, amount, bufferedContributions, bufferedSums);
      }
    ...
    */
    function _updateContributions(
      address contributor,
      uint amount,
      mapping(address => uint) storage contributionType,
      uint sumtype
    )
      internal
      returns (uint)
    {
      uint maxInvestment = IContributorRestrictions(contributorRestrictions).minInvestmentAmount();
      uint refund;
      if(maxInvestment == 0 || IContributorRestrictions(contributorRestrictions).checkMaxInvestment(amount)) {
        contributionType[contributor] = contributionType[contributor].add(amount);
        sumtype = sumtype.add(amount);
        refund = 0;
      } else {
        refund = (contributionType[contributor]).sub(maxInvestment);
        contributionType[contributor] = maxInvestment;
      }
      return refund;
    }

    function _addAffiliate(address user, string memory affiliateLink, uint amount) internal {
      if (bytes(affiliateLink).length > 0) {
        (address affiliate, uint percentage) =
        ...
        if(affiliate != address(0)) {
          if (referral[user] == address(0) || referral[user] == affiliate) {
            ...
            uint claim = amount.mul(percentage).div(100);
    ...
    function _removeAffiliate(address user) internal {
        if (referral[user] != address(0)) {
          totalAffiliateClaim[referral[user]] = totalAffiliateClaim[referral[user]].sub(affiliateClaim[user]);
          affiliateClaim[user] = 0;
          referral[user] = address(0);
        }
    }

    function _checkHardCap(uint amount) internal view returns (bool, uint) {
      bool pass;
      uint refund;
    ...
    function _forwardFunds(address proxy, uint amount) internal {
      IERC20(baseCurrency).transfer(proxy, amount);
    }

    function _retrieveFunds(address proxy, uint amount) internal {
    ...
    *  @return true on success
    */
    function _finishFundraise() internal returns (uint) {
      ...
      if (tokenPrice > 0)
        return fundraiseAmount.div(tokenPrice);
      else {
  ...
  function _fullRefund(address user) internal returns (bool) {
    uint amount = qualifiedContributions[user].add(bufferedContributions[user]);
  ...
  {
    require(
        IERC20(baseCurrency).transfer(contributor, amount),
        "ERC20 transfer failed!"
    );
